...
function AI(ttt) {
	...
	this.solve = function() {
		...
		var state = {
			...
			,nextBoard : this.ttt.nextBoard
			,board : this.deepCopy(this.ttt.state)
		};

		var result = this.minmax(4, state); // 2 depth, computer = player 2
		//this.ttt.nextBoard = [result[3], result[4]];
	...
	this.evaluate = function(state) {
		var score = 0;
		// alert("has player one? " + state.player);
		for(var x = 0; x < 3; x++) {
			for(var y = 0; y < 3; y++) {
				if(this.hasWon(x, y, state)) {
				//	alert("win, player: " + state.player + " " + state.board);
					score += 100;
